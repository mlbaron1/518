...
author: "Márton Sóskuthy: with edits by Mitchi KB"
date: "13/09/2022"
output:
...
#identical:::: s9 <- rename(s9, volume="Voume")
#advantage is that with PIPE you can keep adding more and more 

#syntac: newname = "oldname"
...
#observations -> sometimes OVERWRITING causes problems 
```
...
```{r}
all_s_bay <- all_s %>%
  mutate(bayesian_mixed = ifelse(bayesian== TRUE & mixed_effects == TRUE, TRUE, FALSE))
...
all_s %>%
  group_by(bayesian)%>%
  summarise(n=sum(mixed_effects),
            prop=mean(mixed_effects))
```
